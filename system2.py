...
def weightcalculation(var_evs=[1]):  # 总共几个表, 请写入参数
    ...
    with open('./mark.txt','a',encoding='utf-8') as f:
        for j in range(len(var_evs)):  # 可能要改
            #if ( var_evs[j]<=var_max and var_evs[j]>=var_mean):  # 4
            if ( var_evs[j]<=1 and var_evs[j]>=0.015):
                mark.append(70 - (var_evs[j] - 0.015) / ((var_max - 0.015) / 10))
                print("第", j, "号表状态: 异常,得分: ", mark[len(mark)-1])
                f.write("第%d号表状态: 异常,得分: %d"%(j,mark[len(mark)-1]))
                #f.write("%d"%mark[len(mark)-1])
                level.append(4)
            elif ( var_evs[j]<=0.015 and var_evs[j]>=0.01):  # 3
                mark.append(80 - abs((var_evs[j] - 0.01) / ((var_levelfour - 0.01) / 10)))
...
def read_data(file="D:\\Ji Xun\\数据\\123.xlsx"):
    ...
    for i in range(1,nrows):  #数据不对
        value.append(table.row_values(i)[0:24])
